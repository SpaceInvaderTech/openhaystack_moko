...
RUN wget --no-verbose "https://bootstrap.pypa.io/pip/2.7/get-pip.py" -O /tmp/get-pip.py
RUN python2 /tmp/get-pip.py
RUN rm /tmp/get-pip.py
RUN pip install nrfutil
# mergehex is not included in nrfutil
RUN apt-get install -y libusb-1.0-0
RUN wget --no-verbose "https://nsscprodmedia.blob.core.windows.net/prod/software-and-other-downloads/desktop-software/nrf-command-line-tools/sw/versions-10-x-x/10-23-2/nrf-command-line-tools_10.23.2_amd64.deb" -O /tmp/nrf-command-line-tools.deb
